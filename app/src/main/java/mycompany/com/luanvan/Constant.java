package mycompany.com.luanvan;

public class Constant {

    public static final String CHAT_SERVER_URL = "http://192.168.100.120:3000";
    public static final String URL_SERVER = CHAT_SERVER_URL + "/";
    public static final String URL_IMG = URL_SERVER + "images/monans/";
    public static final String SO_BAN = "soban";
    public static final String TOKEN = "token";
    public static final String STT_BAN_AN = "sttBanAn";
    public static final String STATUS = "status";
    public static final String CALL = "call";
    public static final String SOCKET_VAN_CHUYEN = "goi-mon-van-chuyen-";
    public static final String SOCKET_VAN_CHUYEN_HOAN_THANH = "goi-mon-van-chuyen-hoan-thanh-";
    public static final String SOCKET_DA_THANH_TOAN = "goi-mon-da-thanh-toan-";

    public static final int NOTIFI_ID = 1;


}
